...
class SubbandFiltering(Operator):
    def __init__(self, op_hp, sample_rate):
        ...
        if w == "hann":
            ...
            assert (
                ...
                "hop length must be less than 1/4 of win_length to avoid temporal aliasing"
            )  # (it should work with 1/2, but somehow it doesn't)
        else:
            raise NotImplementedError("window type {} not implemented".format(w))

        self.window_padded = torch.nn.functional.pad(
            self.window, (0, self.n_fft - self.win_length), mode="constant", value=0
        )  # zero padding
    ...
    def apply_stft(self, x):
        ...
        xpadded = torch.nn.functional.pad(
            x, (0, self.win_length)
        )  # bad things happen at the end if you don't pad (because of the centering of torch.sftft and the padded window, I think)
    ...
    def apply_istft(self, X, length=None):
        ...
        x = x[
            ..., self.win_length // 2 :
        ]  # account for extra delay caused by centering the stft (see subband_filtering())
    ...
    def subband_filtering(self, X, H):
        ...
        X = torch.nn.functional.pad(
            X, (H_filt.shape[-1] - 1 - pre_impulse_frames, pre_impulse_frames)
        )  # centering the RIR where the hann window with the direct path at the center is at the origin
    ...
    def update_H(self, rir=None, H=None):
        if rir is not None:
            ...
            raise ValueError(
                "Either rir or H must be specified. This is the informed scenario, so we need to know the filter"
            )

        assert self.H.shape[0] == self.n_fft // 2 + 1, "H.shape: {}, n_fft//2 +1: {}".format(
            self.H.shape, self.n_fft // 2 + 1
        )
...
class BlindSubbandFiltering(SubbandFiltering):
    def __init__(self, op_hp, sample_rate, magnitude_distance=True, H_cplx=False):
        ...
        assert (
            len(multiexp_weighting) == self.num_exponentials
        ), "multiexp_weighting must have the same length as T60_breakpoints"

        if self.fix_EQ_extremes:
            assert T60_breakpoints_init.shape[-1] == (
                len(self.EQ_freqs) - 2
            ), "T60_breakpoints must have the same length as EQ_freqs-2"
            assert multiexp_weighting.shape[1] == (
                len(self.EQ_freqs) - 2
            ), "multiexp_weighting must have the same length as EQ_freqs-2"
        else:
            assert T60_breakpoints_init.shape[-1] == (
                len(self.EQ_freqs)
            ), "T60_breakpoints must have the same length as EQ_freqs"
            assert multiexp_weighting.shape[1] == (
                len(self.EQ_freqs)
            ), "multiexp_weighting must have the same length as EQ_freqs"
        ...
        with torch.no_grad():
            self.phases = (
                torch.rand((self.n_fft // 2 + 1, self.Nf), dtype=torch.float32).to(self.device) * 2 * np.pi - np.pi
            )
    ...
    def design_subband_filter(self):
        ...
        decay_matrix[1:-1] = (
            weights.unsqueeze(-1)
            * decay_breakpoints.unsqueeze(-1)
            ** (-torch.arange(0, Nf).to(decay_breakpoints.device).unsqueeze(0).unsqueeze(0).float())
        ).sum(0)
    ...
    def project_params(self):
        ...
        for k in range(0, len(self.params[1][0])):
            self.params[1][0][k] = torch.clamp(
                self.params[1][0][k], min=10 ** (self.Amin / 20), max=10 ** (self.Amax / 20)
            )  # main exponential must have at least 50% of the weight
            for i in range(1, len(self.params[1])):
                self.params[1][i][k] = torch.clamp(
    ...
    def cons(self, X, length=None):
        ...
        if self.fix_direct_path:
            ...
            )  # fix the direct path to 1 (hard coded for hann window)
            # else:
            # h[0]=-1*(self.win_length/(self.hop_length*2)) #fix the direct path to 1 (hard coded for hann window)
        X_rec = self.stft(h)
        X_rec = X_rec[:, 1:-1]
