...
class CompressedMagSTFT(torch.nn.Module):
    def __init__(self, stft_kwargs, spec_kwargs, inv=False):
        ...
        assert all([k in spec_kwargs for k in ["transform_type", "abs_exponent", "factor"]])

        self._inv = inv

        self.stft_kwargs = OmegaConf.create(stft_kwargs)  # n_fft, hop_length, window_name
        self.spec_kwargs = OmegaConf.create(spec_kwargs)
        window = get_window(self.stft_kwargs.pop("window_name", "hann"), self.stft_kwargs.n_fft)
    ...
    def forward(self, x, inv=False, length=None):
        ...
        if not inv:
            ...
            if x.ndim != 3:
                raise ValueError("Expects a 3D input tensor (batch, freq/real/imag, time)")
            # restore shape
            n_freq = x.shape[1] // 2
