...
class PhonemeEmbeddingLoss(Module):
    ...
    def __init__(self, checkpoint="facebook/wav2vec2-lv-60-espeak-cv-ft", sr=16000, loss=None):
        super().__init__()
        self.sr = sr
        ...
        if self.sr != self.phoneme_predictor.sr:
            self.resampler = torchaudio.transforms.Resample(self.sr, self.phoneme_predictor.sr)
        else:
...
class PhonemeCTCLoss(Module):
    ...
    def _targets_to_ctc(self, target):
        ...
        with torch.no_grad():
            ...
            S = max(lengths)
            ids = [torch.cat([i, i.new_full((S - i.shape[0],), self.blank)]) for i in ids]
