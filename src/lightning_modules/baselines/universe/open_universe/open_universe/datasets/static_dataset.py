...
class NoisyDataset(Dataset):
    ...
    def __init__(
        ...
        if self.clean_available:
            log.info(f"{self.__class__}: path={audio_path} " f"{len(self.file_list)} noisy/clean pairs")
        else:
            self.clean_path = None
            log.info(f"{self.__class__}: path={audio_path} " f"{len(self.file_list)} noisy files, no clean reference")
