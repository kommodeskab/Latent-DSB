...
def prepare(ref_path, deg_path, results_path):
    ...
    if n_results_not_in_path > 0:
        raise ValueError("Some results do not have a corresponding file. Check consistency")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Run evaluation on validation or test dataset")
    parser.add_argument("enhanced_path", type=Path, help="Path to enhanced speech folder")
    parser.add_argument(
        "--ref_path",
        type=Path,
        help="Path to reference clean audio folder. If not provided, " "only non-intrusive metrics are computed.",
    )
    parser.add_argument(
        "--result_dir",
        type=Path,
