...
def make_st_convs(
    ...
    for i in range(len(ds_factors)):
        if i >= num_layers:
            ...
            if use_antialiasing:
                new_block = torch.nn.Sequential(BinomialAntiAlias(rates[i] * 2 + 1), new_block)
...
class ConditionerDecoder(torch.nn.Module):
    def __init__(
        ...
        self.input_conv_block = ConvBlock(n_channels[0] * 2, act_type=act_type, use_weight_norm=use_weight_norm)
        up_modules = [
            ConvBlock(
                c,
                r,
