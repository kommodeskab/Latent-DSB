...
def instantiate_scheduler(config_scheduler, optimizer):
    ...
    scheduler["scheduler"] = instantiate({**lr_sch_kwargs, **{"optimizer": optimizer}}, _recursive_=False)
    return scheduler


def step_scheduler(scheduler, interval, frequency, epoch, step, is_last_batch, loss_val=None):
...
class UniverseGAN(Universe):
    ...
    def init_losses(self, score_model, condition_model, losses, training):
        ...
        self.loss_mrd = gan.MultiResolutionDiscriminator(losses.multi_resolution_discriminator)
    ...
    def model_parameters(self):
        ...
        params = itertools.chain(self.get_score_model().parameters(), self.condition_model.parameters())
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss_disc_f, losses_disc_f_r, losses_disc_f_g = gan.discriminator_loss(y_df_hat_r, y_df_hat_g)
        # 2. MRD
        y_ds_hat_r, y_ds_hat_g, _, _ = self.loss_mrd(target_original, y_est.detach())
        loss_disc_s, losses_disc_s_r, losses_disc_s_g = gan.discriminator_loss(y_ds_hat_r, y_ds_hat_g)

        loss_disc = loss_disc_s + loss_disc_f

        if self.global_step >= self.disc_freeze_step:
            self.manual_backward(loss_disc)
            grad_norm_mpd = torch.nn.utils.clip_grad_norm_(self.loss_mpd.parameters(), self.grad_clip_vals.mpd)
            grad_norm_mrd = torch.nn.utils.clip_grad_norm_(self.loss_mrd.parameters(), self.grad_clip_vals.mrd)
            opt_disc.step()
            if has_schedulers:
        ...
        if self.loss_aux is not None:
            ...
            if self.global_step >= start_step:
                w = min(1.0, (self.global_step - start_step) / warmup_steps) if warmup_steps > 0 else 1.0
        ...
        # 4. MPD loss
        y_df_hat_r, y_df_hat_g, fmap_f_r, fmap_f_g = self.loss_mpd(target_original, y_est)
        loss_fm_f = gan.feature_loss(fmap_f_r, fmap_f_g)
        loss_gen_f, losses_gen_f = gan.generator_loss(y_df_hat_g)

        # 5. MRD loss
        y_ds_hat_r, y_ds_hat_g, fmap_s_r, fmap_s_g = self.loss_mrd(target_original, y_est)
        ...
        grad_norm_score = torch.nn.utils.clip_grad_norm_(self.get_score_model().parameters(), self.grad_clip_vals.score)

        grad_norm_cond = torch.nn.utils.clip_grad_norm_(self.condition_model.parameters(), self.grad_clip_vals.cond)
        opt_score.step()
    ...
    def configure_optimizers(self):
        ...
        for submod in [self.get_score_model(), self.condition_model]:
            excluded += [p for (name, p) in submod.named_parameters() if pick_excluded(name) and p.requires_grad]
            others += [p for (name, p) in submod.named_parameters() if not pick_excluded(name) and p.requires_grad]
        ...
        optimizer_disc = instantiate(self.opt_kwargs.discriminator, params=params_disc, _recursive_=False)

        self.grad_clip_vals = self.opt_kwargs.grad_clip_vals
        self.grad_clipper = None

        if self.schedule_kwargs is not None:
            scheduler_gen = instantiate_scheduler(self.schedule_kwargs.get("generator"), optimizer_gen)
            scheduler_disc = instantiate_scheduler(self.schedule_kwargs.get("discriminator"), optimizer_disc)
