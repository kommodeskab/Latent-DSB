...
class UniverseLoRA(EnhancementBaseModel):
    def __init__(
        ...
        if self.ema_decay > 0.0:
            self.ema = ExponentialMovingAverage(self.trainable_parameters(), decay=self.ema_decay)
    ...
    def hifi_gan_loss(self, y_est, target_original):
        # MPD loss
        y_df_hat_r, y_df_hat_g, fmap_f_r, fmap_f_g = self.model.loss_mpd(target_original, y_est)
        loss_fm_f = gan.feature_loss(fmap_f_r, fmap_f_g)
        loss_gen_f, losses_gen_f = gan.generator_loss(y_df_hat_g)

        # MRD loss
        y_ds_hat_r, y_ds_hat_g, fmap_s_r, fmap_s_g = self.model.loss_mrd(target_original, y_est)
    ...
    def compute_losses(self, enh, target, train=True):
        ...
        if self.use_hifigan_loss:
            loss_gen_f, loss_gen_s, loss_fm_f, loss_fm_s = self.hifi_gan_loss(enh, target)
    ...
    def partial_diffusion(self, mix, rng=None):
        ...
        if self.partial_diffusion_random_steps:
            n_steps = torch.randint(low=2, high=n_steps + 1, size=(1,), generator=rng).item()
        ...
        # compute parameters
        sigma_ratio = self.model.diff_kwargs.sigma_max / self.model.diff_kwargs.sigma_min
        ...
        # conditioning
        cond, aux_signal, aux_latent = self.model.condition_model(mix, x_wav=mix_wav, train=True)
        ...
        for n in range(0, n_steps - 1):
            with torch.set_grad_enabled(n >= n_steps - self.n_steps_backprop):
                ...
                x = x + sigma[..., None, None] ** 2 * eta[..., None, None] * score + beta[..., None, None] * z
    ...
    def train(self, mode=True, no_ema=False):
        ...
        if not self._error_loading_ema:
            if mode is False and not no_ema:
                # eval
                self.ema.store(self.trainable_parameters())  # store current params in EMA
                self.ema.copy_to(self.trainable_parameters())  # copy EMA parameters over current params for evaluation
            else:
                # train
                if self.ema.collected_params is not None:
                    self.ema.restore(self.trainable_parameters())  # restore the EMA weights (if stored)
        return res
