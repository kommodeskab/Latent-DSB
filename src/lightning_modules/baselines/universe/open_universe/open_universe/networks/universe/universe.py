...
class Universe(pl.LightningModule):
    def __init__(
        ...
        if self.ema_decay > 0.0:
            self.ema = ExponentialMovingAverage(self.model_parameters(), decay=self.ema_decay)
    ...
    def _get_edm_weights(self, sigma):
        level_db = self.edm_kwargs.get("data_level_db", self.normalization_kwargs.get("level_db", 0.0))
    ...
    def print_count(self):
        num_params_score_and_cond = utils.count_parameters(self.condition_model) + utils.count_parameters(
            self.score_model
        )
    ...
    def adapt_time_sampling(self, x):
        with torch.no_grad():
            if not hasattr(self, "pr_cum"):
                ...
                pr_cum = torch.broadcast_to(self.pr_cum, (x.shape[0], self.pr_cum.shape[0]))
                ...
                for i in range(self.val_score_values.shape[0]):
                    ...
                    time = torch.where(
                        torch.logical_and(dice_roll >= pr_cum[:, i], dice_roll < pr_cum[:, i + 1]),
                        cand,
                        time,
                    )
        return time

    def sample_sigma(self, x, time_sampling="time_uniform", t_min=0.0, t_max=1.0, rng=None):
        # sample the variance
        ...
        if time_sampling == "time_uniform":
            ...
            try:
                ...
                alpha = torch.tensor(0.95)  # we want to use 100 * alpha % of the distribution
    ...
    def compute_losses(
        ...
        if train:
            if self.losses_kwargs.weights.latent > 0.0 and h_est is not None:
                ...
                mel_target = mel_target / torch.linalg.norm(mel_target, dim=(-2, -1), keepdim=True).clamp(min=1e-5)
    ...
    def training_step(self, batch, batch_idx):
        ...
        (mix, target), *stats = self.normalize_batch((mix, target), norm=self.normalization_norm)
    ...
    def validation_step(self, batch, batch_idx, dataset_i=0):
        ...
        self.log("val/score", l_score, on_epoch=True, sync_dist=True, batch_size=batch_size)
        for i in range(self.val_kwargs.n_bins):
            self.log(
    ...
    def configure_optimizers(self):
        ...
        excluded = [p for (name, p) in self.named_parameters() if pick_excluded(name) and p.requires_grad]
        others = [p for (name, p) in self.named_parameters() if not pick_excluded(name) and p.requires_grad]
        ...
        if self.schedule_kwargs is not None:
            if "scheduler" not in self.schedule_kwargs:
                scheduler = instantiate({**self.schedule_kwargs, **{"optimizer": optimizer}})
            else:
    ...
    def train(self, mode=True, no_ema=False):
        ...
        if not self._error_loading_ema:
            if mode is False and not no_ema:
                ...
                self.ema.copy_to(self.model_parameters())  # copy EMA parameters over current params for evaluation
            else:
                # train
                if self.ema.collected_params is not None:
                    self.ema.restore(self.model_parameters())  # restore the EMA weights (if stored)
        return res
