...
class LinearProj(torch.nn.Module):
    def __init__(self, in_dim, out_dim=None, use_weight_norm=False):
        ...
        if out_dim is None:
            out_dim = in_dim
        self.proj = cond_weight_norm(torch.nn.Conv1d(in_dim, out_dim, 1), use=use_weight_norm)
...
class BinomialAntiAlias(torch.nn.Module):
    ...
    def forward(self, x):
        inch = x.shape[1]
        weights = torch.broadcast_to(self.weights[None, None, :], (inch, 1, self.weights.shape[0]))
        x = F.conv1d(x, weights, padding="same", groups=inch)
        return x
...
class PReLU_Conv(torch.nn.Module):
    def __init__(
        ...
        if act_type == "snake":
            ...
            self.prelu = bigvgan.AliasFreeSnake(in_channels, alpha_logscale=True, beta=True)
        elif act_type == "prelu":
...
class ConvBlock(torch.nn.Module):
    ...
    def __init__(
        ...
        if rate_change_dir not in ["up", "down", "none"]:
            raise ValueError("The rate_change_dir value should be one of 'up' or 'down'")

        if rate_change_dir in ["up", "down"] and rate_change is None:
            raise ValueError("The rate_change should be specified when using for down/upsampling")
        ...
        if signal_cond_type == "x-attention":
            self.signal_cond_proj = LocalCrossAttention(d_model=n_channels, num_memory=20, block_len=100)
        elif signal_cond_type == "linear":
            self.signal_cond_proj = LinearProj(n_channels, use_weight_norm=use_weight_norm)
    ...
    def forward(
        ...
        if res is not None:
            if self.rate_change_dir != "down":
                ...
                raise ValueError("The residual input is not allowed for downsampling blocks")
