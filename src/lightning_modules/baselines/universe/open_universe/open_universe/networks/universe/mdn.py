...
class MixtureDensityNetworkLoss(torch.nn.Module):
    def __init__(
        ...
        self.eps = eps
        self.layer_norm = torch.nn.LayerNorm((est_channels, int(sampling_rate * sample_len_s)))
        self.conv = PReLU_Conv(est_channels, 3 * n_comp * tgt_channels, kernel_size=3, padding="same")
    ...
    def sample(logit, mean, std, random=True):
        ...
        if random:
            ...
            z = mean.new_empty(i_cmpnts.shape).normal_(generator=rng)  # (B, 1, out_dim, T)
            out = z * std.gather(1, i_cmpnts) + mean.gather(1, i_cmpnts)  # (B, 1, out_dim, T)
