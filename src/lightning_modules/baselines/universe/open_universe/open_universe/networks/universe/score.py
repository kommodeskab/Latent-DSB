...
class ScoreEncoder(torch.nn.Module):
    def __init__(
        ...
        if self.extra_conv_block:
            self.ds_modules.append(ConvBlock(oc, act_type=act_type, use_weight_norm=use_weight_norm))
...
class ScoreDecoder(torch.nn.Module):
    def __init__(
        ...
        n_channels = [input_channels * 2 ** (len(up_factors) - i - 1) for i in range(len(up_factors))]
        ...
        if self.extra_conv_block:
            ...
            self.up_modules.append(ConvBlock(oc, act_type=act_type, use_weight_norm=use_weight_norm))
...
class ScoreNetwork(torch.nn.Module):
    def __init__(
        ...
        self.input_conv = torch.nn.Conv1d(input_channels, n_channels, kernel_size=fb_kernel_size, padding="same")
    ...
    def forward(self, x, sigma, cond):
        ...
        s = self.decoder(h, noise_cond=g, input_cond=cond, residuals=residuals, lengths=lengths)
