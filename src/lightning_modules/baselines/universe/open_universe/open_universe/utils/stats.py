...
def signal_median(signal):
    ...
    pad_bins = torch.broadcast_to(torch.arange(n, device=signal.device)[None, :], (min_indices.shape[0], n))
    ...
    median_signal = torch.stack([signal[select[i], i, :] for i in range(signal.shape[1])], dim=0)
