...
class LinearWarmupCosineAnnealingLR(torch.optim.lr_scheduler.CosineAnnealingLR):
    ...
    def get_lr(self):
        ...
        if self.last_epoch <= self.T_warmup:
            return [
                self.eta_min + (base_lr - self.eta_min) * self.last_epoch / self.T_warmup
        ...
        if self.last_epoch <= self.T_cosine:
            ...
            return [
                self.eta_min + (base_lr - self.eta_min) * (1 + math.cos((num) * math.pi / denom)) / 2
                for base_lr, group in zip(self.base_lrs, self.optimizer.param_groups)
            ]
        elif (num - 1 - denom) % (2 * denom) == 0:
            return [
                group["lr"] + (base_lr - self.eta_min) * (1 - math.cos(math.pi / denom)) / 2
                for base_lr, group in zip(self.base_lrs, self.optimizer.param_groups)
            ]
        return [
