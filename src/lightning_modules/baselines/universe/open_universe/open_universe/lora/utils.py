...
def get_adapter(module: torch.nn.Module, rank: int, alpha: Optional[float] = None) -> Union[LoraConv1d, LoraLinear]:
    """
...
def freeze_parameters_except_lora_and_bias(
    ...
    for name, param in module.named_parameters():
        is_trainable = "lora_" in name or any([s in name for s in train_names]) or (train_biases and "bias" in name)
