...
class LoraConv1d(torch.nn.Module):
    ...
    def __init__(self, module: torch.nn.Module, rank: int, alpha: Optional[float] = None):
        super().__init__()
        ...
        if w.shape[1] < rank or w.shape[0] < rank:
            raise ValueError("The rank should be smaller than the input and output size")

        self.lora_weight_a = torch.nn.Parameter(w.new_zeros(w.shape[0], rank))
        self.lora_weight_b = torch.nn.Parameter(w.new_zeros(rank, w.shape[1] * w.shape[2]).normal_())
...
class LoraConvTranspose1d(torch.nn.Module):
    ...
    def forward(self, input):
        weight = self._get_weights()
        return F.conv_transpose1d(input, weight, bias=self.conv.bias, **self._get_kwargs())
