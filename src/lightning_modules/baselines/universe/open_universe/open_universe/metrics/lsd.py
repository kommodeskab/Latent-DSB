...
def log_spectral_distance(
    ...
    **stft_kwargs,
    ...
    if window is None:
        window = torch.hann_window(n_fft, periodic=True, dtype=input.dtype, device=input.device)

    if p is None or p <= 0:
        raise ValueError(f"p must be a positive number, but got p={p}")

    if scale_invariant:
        scaling_factor = torch.sum(input * target, -1, keepdim=True) / (torch.sum(input**2, -1, keepdim=True) + eps)
    else:
        scaling_factor = 1.0
