...
class PLCMOSEstimator:
    def __init__(self, model_version=2, embed_rounds=15, num_threads=None):
        ...
        model_paths = {
            ...
            "0": [
                (curdir / "models/plcmos_v1_intrusive.onnx", 768),
                (curdir / "models/plcmos_v1_nonintrusive.onnx", 999999999999),
            ],
    ...
    def stana(self, sig, sr, wind, hop, synth=False, center=False):
        ...
        return as_strided(sigpad, shape=(nframe, fsize), strides=(sig.itemsize * hsize, sig.itemsize)) * wind
    ...
    def run(self, audio_degraded, sr_degraded, audio_clean=None, combined=True, return_intermediate_scores=False):
        ...
        for i in range(self.embed_rounds):
            ...
            if audio_clean is not None:
                ...
                if self.use_embed:
                    onnx_inputs = {
                        ...
                        "rater_embed": np.array(rater_embed, dtype=np.float32).reshape(1, -1),
                    }
                else:
            ...
            if combined and (self.sessions[0] is not None or self.sessions[1] is None) and audio_clean is not None:
                mos /= 2.0
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model-ver", type=int, default=2, help="Model version to use. Defaults to the final v2 model."
    )
