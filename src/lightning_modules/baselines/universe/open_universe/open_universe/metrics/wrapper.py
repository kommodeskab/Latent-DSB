...
def cached_resample(orig_fs, new_fs, cache={}, **signals):
    ...
    for label, signal in signals.items():
        if not (new_fs in cache and label in cache[new_fs]):
            cache[new_fs][label] = torchaudio.functional.resample(signal, orig_freq=orig_fs, new_freq=new_fs)
...
class Metrics:
    ...
    def _pesq_base(self, ref, deg, fs, mode, cache={}):
        sig, cache = cached_resample(orig_fs=fs, new_fs=16000, cache=cache, ref=ref, deg=deg)
        return (
    ...
    def lps(self, ref, deg, fs, cache={}):
        ...
        sig, cache = cached_resample(orig_fs=fs, new_fs=self._lps.sr, cache=cache, ref=ref, deg=deg)
    ...
    def dnsmos(self, ref, deg, fs, cache={}):
        ...
        val = {m: float(val[m]) for m in ["OVRL_raw", "SIG_raw", "BAK_raw", "OVRL", "SIG", "BAK"]}
        return val, cache
    ...
    def sdr(self, ref, deg, fs, cache={}):
        ...
        val = fast_bss_eval.sdr(ref, deg, zero_mean=False, return_perm=False, clamp_db=100)
    ...
    def si_sdr(self, ref, deg, fs, cache={}):
        ...
        val = fast_bss_eval.si_sdr(ref, deg, zero_mean=False, return_perm=False, clamp_db=100)
        return {"si-sdr": float(val.mean())}, cache

    def __call__(self, fs, degraded, reference=None, skip_list=None, skip_unknown_metrics=True):
        """
        Parameters
        ----------
        fs : int
