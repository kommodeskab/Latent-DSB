...
class EvalMetrics(torch.nn.Module):
    ...
    def __init__(self, pesq_mode="wb", lps=False, audio_fs=16000):
        ...
        if self.eval_fs != self.audio_fs:
            self.resampler = torchaudio.transforms.Resample(orig_freq=self.audio_fs, new_freq=self.eval_fs)
        else:
    ...
    def forward(self, est, ref):
        ...
        for ii in range(est.shape[0]):
            try:
                # PESQ
                metrics_lists["pesq"].append(pesq(self.eval_fs, ref[ii, 0], est[ii, 0], self.mode))
