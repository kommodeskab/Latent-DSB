...
from src.lightning_modules.baselines.sgmse.sgmse.util.registry import Registry
    ...
    def reverse(oself, score_model, probability_flow=False):
        ...
        class RSDE(oself.__class__):
            ...
            def rsde_parts(self, x, y, t, *args):
                ...
                score_drift = (
                    -(sde_diffusion[:, None, None, None] ** 2) * score * (0.5 if self.probability_flow else 1.0)
                )
            ...
            def discretize(self, x, y, t, stepsize):
                ...
                rev_f = f - G[:, None, None, None] ** 2 * score_model(x, y, t) * (0.5 if self.probability_flow else 1.0)
...
class OUVESDE(SDE):
    ...
    def add_argparse_args(parser):
        ...
        parser.add_argument(
            "--N", type=int, default=30, help="The number of timesteps in the SDE discretization. 30 by default"
        )
    ...
    def _std(self, t):
        ...
        return torch.sqrt(
            (sigma_min**2 * torch.exp(-2 * theta * t) * (torch.exp(2 * (theta + logsig) * t) - 1) * logsig)
            / (theta + logsig)
        )
...
class SBVESDE(SDE):
    ...
    def add_argparse_args(parser):
        parser.add_argument(
            "--N", type=int, default=50, help="The number of timesteps in the SDE discretization. 50 by default"
        )
        parser.add_argument(
            "--k", type=float, default=2.6, help="Parameter of the diffusion coefficient. 2.6 by default."
        )
        parser.add_argument(
            "--c", type=float, default=0.4, help="Parameter of the diffusion coefficient. 0.4 by default."
        )
        parser.add_argument(
            "--eps", type=float, default=1e-8, help="Small constant to avoid numerical instability. 1e-8 by default."
        )
    ...
    def _sigmas_alphas(self, t):
        ...
        alpha_T = torch.ones_like(t)
        sigma_t = torch.sqrt((self.c * (self.k ** (2 * t) - 1.0)) / (2 * torch.log(torch.tensor(self.k))))  # Table 1
        sigma_T = torch.sqrt(
            (self.c * (self.k ** (2 * self.T) - 1.0)) / (2 * torch.log(torch.tensor(self.k)))
        )  # Table 1
