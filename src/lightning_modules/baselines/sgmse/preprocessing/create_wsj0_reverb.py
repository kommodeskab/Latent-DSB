...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--target_dir", type=str, required=True, help="Path to the target directory for saving WSJ0-REVERB."
    )
    ...
    speech_lists = {
        split: sorted(glob(f"{os.path.join(args.wsj0_dir, dic_split[split])}/**/*.wav")) for split in splits
    }
    sample_rate = 16000
    ...
    for i_split, split in enumerate(splits):
        ...
        for i_sample in tqdm(range(real_nb_samples)):
            if not i_sample % NB_SAMPLES_PER_ROOM:  # Generate new room
                ...
                room_dim = np.array(
                    [np.random.uniform(DIM_RANGE[2 * n], DIM_RANGE[2 * n + 1]) for n in range(3)]
                )  # Draw Dimensions
                center_mic_position = np.array(
                    [np.random.uniform(MIN_DISTANCE_TO_WALL, room_dim[n] - MIN_DISTANCE_TO_WALL) for n in range(3)]
            ...
            dry = np.pad(
                dry, ((0, int(0.5 * sample_rate)), (0, 0)), mode="constant", constant_values=0
