...
from src.lightning_modules.baselines.sgmse.sgmse.util.other import set_torch_cuda_arch_list

set_torch_cuda_arch_list()
torch.set_float32_matmul_precision("high")

from src.lightning_modules.baselines.sgmse.sgmse.backbones.shared import BackboneRegistry
from src.lightning_modules.baselines.sgmse.sgmse.data_module import SpecsDataModule
from src.lightning_modules.baselines.sgmse.sgmse.sdes import SDERegistry
...
if __name__ == "__main__":
    ...
    trainer_parser.add_argument(
        "--accelerator", type=str, default="gpu", help="Supports passing different accelerator types."
    )
    ...
    if logger != None:
        callbacks = [
            ModelCheckpoint(dirpath=join(args.log_dir, str(logger.version)), save_last=True, filename="{epoch}-last")
        ]
        callbacks += [
            ModelCheckpoint(
