...
from .operators.subband_filtering import BlindSubbandFiltering
...
class Tester:
    ...
    def load_latest_checkpoint(self):
        ...
        try:
            ...
            list_ids = [int(id_regex.search(weight_path).groups()[0]) for weight_path in list_weights]
    ...
    def sample_unconditional(self, mode):
        audio_len = (
            ...
            if "audio_len" not in self.args.tester.unconditional.keys()
        ...
        if not self.in_training:
            for i in range(len(preds)):
                path_generated = utils_logging.write_audio_file(
                    preds[i], self.args.exp.sample_rate, f"unconditional_{i}", path=self.paths["unconditional"]
                )
    ...
    def test_dereverberation(self, mode, blind=False):
        ...
        for seg, batch_name, file_name in tqdm(self.test_set):
            seg = torch.from_numpy(seg).float().to(self.device).unsqueeze(0)
            y = (
                self.args.tester.posterior_sampling.warm_initialization.scaling_factor * seg / seg.std()
            )  # Normalize the input to match sigma_data of dataset
            ...
            with torch.no_grad():
                if blind:  # Initialize operator
                    ...
                    operator_blind = BlindSubbandFiltering(
                        self.args.tester.informed_dereverberation.op_hp, sample_rate=self.args.exp.sample_rate
                    )
                    with torch.no_grad():
                        operator_blind.update_H(use_noise=True)
                else:
                    raise NotImplementedError(
                        "Only blind dereverberation is implemented for now, please set blind to True"
                    )

            pred = self.sampler.predict_conditional(y, operator_blind, shape=(1, seg.shape[-1]), blind=blind)

            path_reconstructed = utils_logging.write_audio_file(
                pred, self.args.exp.sample_rate, os.path.basename("files")[:-4], path=self.paths[mode + "reconstructed"]
            )
