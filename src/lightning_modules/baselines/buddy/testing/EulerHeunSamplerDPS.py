...
class EulerHeunSamplerDPS(EulerHeunSampler):
    ...
    def initialize_x(self, shape, device, schedule):
        if self.args.tester.posterior_sampling.warm_initialization.mode == "none":
            ...
            x = (
                self.args.tester.posterior_sampling.warm_initialization.scaling_factor * self.y.clone() / self.y.std()
                + schedule[0] * torch.randn(shape).to(device)
            )
            ...
            x_pred = (
                torch.from_numpy(istft(Z, size=stft_options["size"], shift=stft_options["shift"]))
                .to(self.y.device)
                .type(self.y.dtype)
            )
    ...
    def optimize_op(self, x_den, t):
        ...
        for _ in range(self.args.tester.posterior_sampling.blind_hp.op_updates_per_step):
            ...
            if self.RIR_noise_regularization_loss is not None:
                ...
                t_op = max(
                    min(t, self.args.tester.posterior_sampling.RIR_noise_regularization.crop_sigma_max),
                    self.args.tester.posterior_sampling.RIR_noise_regularization.crop_sigma_min,
                )
                rir_noisy = rir_time + t_op * rir_noise
                reg_loss = self.RIR_noise_regularization_loss(
                    rir_time, rir_noisy.detach()
    ...
    def predict(self, shape, device, blind=False):
        ...
        **kwargs,
        ...
        if blind:
            ...
            self.optimizer_operator = torch.optim.Adam(
                self.operator.params + self.operator.params_phases,
            ...
            self.RIR_noise_regularization_loss = get_loss(
                self.args.tester.posterior_sampling.RIR_noise_regularization.loss, operator=self.operator
            )
