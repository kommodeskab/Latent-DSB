...
def downsample_1d(x, k=None, factor=2, gain=1):
    ...
    k = k / np.sum(k) * gain
    p = k.shape[0] - factor
    return upfirdn1d(x, torch.tensor(k, device=x.device), down_y=factor, pad_y0=(p + 1) // 2, pad_y1=(p // 2))
...
def upsample_1d(x, k=None, factor=2, gain=1):
    ...
    p = k.shape[0] - factor
    return upfirdn1d(
        x, torch.tensor(k, device=x.device), up_y=factor, pad_y0=(p + 1) // 2 + factor - 1, pad_y1=(p // 2)
    )
