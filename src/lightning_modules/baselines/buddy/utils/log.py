...
def do_stft(noisy, clean=None, win_size=2048, hop_size=512, device="cpu", DC=True):
    ...
    stft_signal_noisy = torch.stft(
        noisy, win_size, hop_length=hop_size, window=window, center=False, return_complex=False
    )
    ...
    if clean != None:
        ...
        stft_signal_clean = torch.stft(
            clean, win_size, hop_length=hop_size, window=window, center=False, return_complex=False
        )
...
def plot_loss_by_sigma(sigma_means, sigma_stds, sigma_bins):
    df = pd.DataFrame.from_dict({"sigma": sigma_bins, "loss": sigma_means, "std": sigma_stds})
