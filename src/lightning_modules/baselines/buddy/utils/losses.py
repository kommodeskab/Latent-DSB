...
def get_loss(loss_args, operator=None):
    ...
    if hasattr(loss_args, "loss_1"):  # We have a hybrid of multiple losses
        return lambda x, x_hat: torch.sum(
            torch.stack(
                [get_loss(getattr(loss_args, key), operator=operator)(x, x_hat) for key in list(loss_args.keys())]
            )
        )
        ...
        if "stft" in loss_args.name:
            ...
            def loss_fn(x, x_hat, freq_weighting=None):
                ...
                if loss_args.name == "l2_stft_sum":
                    ...
                    assert (
                        compression_factor is not None and compression_factor > 0.0 and compression_factor <= 1.0
