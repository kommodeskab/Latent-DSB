...
class ESDSBCallback(Callback):
    ...
    def on_train_start(self, trainer : Trainer, pl_module : ESDSB):
        ...
        if self.what_media == 'image':
            ...
            logger.log_audio(
                key = 'Initial Samples',
                audios = audios,
                sample_rate = [self.sample_rate] * 32,
                caption = caption,
                step = pl_module.global_step,
            )
        
    def visualize_samples(self, pl_module : ESDSB):
        step = pl_module.global_step
        # plot some results        
    ...
    def refresh_cache(self, pl_module : ESDSB, direction : DIRECTIONS, training : bool) -> None:
