...
from src.flop_utils import calculate_mamba2_block_flops
...
class CountFlopsExtra(ExtraMetricOutput):
    ...
    def __call__(
        self, pl_module: DSB, outputs: StepOutput, batch: UnpairedAudioBatch, batch_idx: int
        ...
        total_flops = torch_flops + mamba_flops
        ...
        if self.verbose:
            ...
            print(f"True Total: {(torch_flops + mamba_flops) / 1e9:.2f} Billion\n")

        return {self.out_key: (total_flops, torch_flops, mamba_flops, zero_flop_ops)}
